...
from api.v1.module.serializers.terms_serializer import *
...
class TermsPagination(PageNumberPagination):
    page_size = 10 
    def get_paginated_response(self, data):
        total_items = self.page.paginator.count
        if not total_items:
            return Response({'message':'Terms no found' , 'code':400 , 'data': {} , 'extra':{}})
        if self.page_size:
            total_page = math.ceil(total_items/self.page_size)
        message = "Terms list fetch successfully"
        return Response({'message':message , 'code':200 , 'data':data , 'extra': {'count':total_items , 'total': total_page , 'page_size': self.page_size}})

class TermsModelViewSet(viewsets.ModelViewSet):
    queryset = Terms.objects.all().order_by('-id')
    serializer_class = TermsSerializer
    pagination_class = TermsPagination
    ...
    def get_queryset(self):
        try:
            ...
            message = "Terms lsit fetch successfully"
            return response_handler(message=message, code=400 , data= {})
        
    def create(self, request, *args, **kwargs):
        try:
            response = super().create(request , *args , **kwargs)
            message = "Terms create successfully"
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            message = "Terms data retrived successfully"
            return response_handler(message= message , code= 200 , data=serializer.data)
        except NotFound:
            return response_handler(message="Terms not found" , code = 400 , data = {})
