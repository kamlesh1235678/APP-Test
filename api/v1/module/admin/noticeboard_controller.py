...
from api.v1.module.serializers.noticeboard_serializer import *
...
class NoticeBoardModelViewSet(viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        try:
            ...
            message = "Notice Board lsit fetch successfully"
            return response_handler(message=message, code=400 , data= {})
        
    def create(self, request, *args, **kwargs):
        try:
            response = super().create(request , *args , **kwargs)
            message = "Notice Board create successfully"
    ...
    def retrieve(self, request, *args, **kwargs):
        try:
            ...
            message = "Notice Board data retrived successfully"
            return response_handler(message= message , code= 200 , data=serializer.data)
        except NotFound:
            return response_handler(message="NoticeBoard no found" , code = 400 , data={})
    
    def destroy(self, request, *args, **kwargs):
        try:
            instance = self.get_object()
            # for delete file folder deltion 
            notice_folder = os.path.join(settings.MEDIA_ROOT, "notice_board", f"notice_{instance.id}")
            if os.path.exists(notice_folder):
                shutil.rmtree(notice_folder)
            # instance deletion
            self.perform_destroy(instance)
            message = "Notice Board delete successfully"
            return response_handler(message= message , code = 200 , data= {})
        except:
            message = "Notice Board no found"
            return response_handler(message= message , code = 400 , data= {})